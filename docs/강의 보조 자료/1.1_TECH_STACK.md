...
## 문서 목적
이 문서는 현재 플랫폼 구현에 사용되는 기술 스택과 선택 이유를 요약하기 위한 문서입니다.  
실습 전 기술 맥락을 빠르게 맞추는 체크포인트로 사용합니다.
...
## 핵심 기술 선택 이유
- Firebase Auth: 간단한 OAuth 인증, 보안 규칙 연계
- Firestore: 실시간 데이터 구독, 자동 확장
- AWS Amplify: Git 기반 배포, SSL/CDN 제공
- Vite + Tailwind: 빠른 개발/빌드 경험
- MQTT: 센서 데이터 실시간 수신

## 레이어별 스택
```mermaid
flowchart TB
  UI[Vite + Tailwind] --> Auth[Firebase Auth]
  UI --> DB[Firestore]
  UI --> MQTT[MQTT Broker]
  UI --> Deploy[AWS Amplify]
  API[Phase 2 Backend] --> SQL[(MySQL/PostgreSQL)]
```
