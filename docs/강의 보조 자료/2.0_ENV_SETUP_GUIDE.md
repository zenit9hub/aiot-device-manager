...
## 문서 목적
이 문서는 실습 환경을 빠르게 준비하기 위한 실행형 체크리스트 문서입니다.  
로컬 실행과 배포 전 필수 환경값을 동일 기준으로 맞추는 데 사용합니다.
...
├── aiot-device-manager-be-working/   # 백엔드 (feat/backend-2.0-working 브랜치 기준)
├── docs/                             # 교육 문서
├── README.md
└── amplify.yml
```
...
## 7.4 환경 변수 템플릿
### 프론트엔드 (`aiot-dvc-mgr-fe-2.0-refactoring/.env`)
```env
VITE_FIREBASE_API_KEY=...
