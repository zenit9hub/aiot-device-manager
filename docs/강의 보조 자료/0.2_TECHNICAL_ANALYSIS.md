...
## 문서 목적
이 문서는 기술 선택의 근거와 제약을 정리해, 왜 현재 구조를 채택했는지를 설명하기 위한 문서입니다.  
강의 중 기술 의사결정 토론의 기준 자료로 사용합니다.
...
MVP1을 빠르게 완성하기 위한 기술 선택 배경과 제약을 정리합니다.
...
## 기술 선택 원칙
- 상용 Managed 서비스 우선
- FE 단에서 직접 사용 가능한 SDK 중심
- Phase 2 확장 시 재사용 가능한 구조

## 대안 비교 요약
- Auth: Firebase Auth (SDK 완성도, 규칙 연계)
- DB: Firestore (실시간, 보안 규칙)
- Hosting: AWS Amplify (Git 기반 배포)
- Realtime: MQTT Broker (웹소켓)

## 기술 결정 흐름
```mermaid
flowchart TD
  A[요구사항] --> B{백엔드 운영 가능?}
  B -- 아니오 --> C[Managed 서비스 중심 설계]
  B -- 예 --> D[커스텀 백엔드 고려]
  C --> E[Firebase Auth/Firestore]
  C --> F[AWS Amplify Hosting]
  C --> G[MQTT over WSS]
  D --> H[Express/Nest + SQL]
```

## 리스크 및 대응
| 리스크 | 영향 | 대응 |
| --- | --- | --- |
| Firestore 권한 오류 | 인증 전 데이터 접근 실패 | 로그인 전에는 데모 데이터만 노출 |
| WSS 브로커 인증서 문제 | MQTT 연결 실패 | 공용 브로커 교체 가능 구조 |
| 배포 환경 변수 누락 | 기능 미동작 | Amplify 환경변수 체크리스트 제공 |
