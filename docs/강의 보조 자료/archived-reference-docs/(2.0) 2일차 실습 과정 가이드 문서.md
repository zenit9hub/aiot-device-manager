...
# 🎓 AIoT Device Manager - 모던 클라우드 플랫폼 구축 및 배포 실습
...
MVP 플랫폼 개발을 위한 **"최소 최적의 기술을 활용한 구축 방법"**에 대한 가이드를 제시합니다.
...
## 🎓 AIoT Device Manager Frontend 구축 실습
...
- Firebase project
	- Authentication
		- 승인된 도메인 메뉴를 통해 도메인 추가: localhost, (amplify url)
	- Firestore enabled

- ##### 서울 기온 MQTT 퍼블리셔
	- 센서 디바이스 시뮬레이터 역할
		- AccuWeather에서 서울의 현재 기온을 웹 스크래핑하여 MQTT 브로커로 발행하는 모니터링 시스템을 센서 디바이스로 활용.
	- GitHub: https://github.com/zenit9hub/seoul-weather-mqtt
		- 날씨 조회 간격: 1분 (60초) - 실제 온도 가져오기
		- MQTT 발행 간격: 10초 - 변화된 온도 데이터 발행

- ##### MQTTX 클라이언트
	- 센서 디바이스의 토픽 메시지 정상 여부 체크 및 테스트 메시지 발송 용도
		- [[(라 1.1) 🛠️ MQTTX 클라이언트 설치 및 셋업 → 브로커 설정, 구독, 발행 테스트]]
...
1. **Clone the repository**
	- https://github.com/zenit9hub/aiot-device-manager-fe.git
```bash
git clone <repository-url>
cd aiot-device-manager-fe
```
...
**"모던클라우드 기반 서비스 플랫폼 설계, 구축 및 배포"** 교육과정에 대한 문의처:
flitsky@naver.com
