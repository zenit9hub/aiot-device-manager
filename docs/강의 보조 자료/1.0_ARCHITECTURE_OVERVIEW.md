...
## 문서 목적
이 문서는 Phase 1/Phase 2 관점에서 시스템 구조를 요약해 설명하기 위한 아키텍처 개요 문서입니다.  
강의 보조 자료의 아키텍처 입문용 레이어로 사용합니다.
...
본 아키텍처는 **Phase 1(Serverless MVP)**을 빠르게 완성하고, **Phase 2(Backend 연동)**으로 확장할 수 있도록 설계되었습니다.

## 설계 원칙
- FE 중심 구조: SDK로 인증/데이터 직접 접근
- 최소 인프라: 상용 서비스 결합
- 확장성: 백엔드 추가 시 FE 구조 유지

## 시스템 컨텍스트
```mermaid
flowchart LR
  User --> FE[SPA Frontend]
...
flowchart LR
  ...
  API --> SQL[(SQL DB)]
  FE --> Store[Firestore (Realtime View)]
```
