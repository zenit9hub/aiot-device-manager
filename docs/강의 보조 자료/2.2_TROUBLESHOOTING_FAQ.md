...
## 문서 목적
이 문서는 실습 중 자주 발생하는 인증/데이터/배포 이슈의 원인과 대응 절차를 빠르게 찾기 위한 문서입니다.  
운영 중단 시간을 줄이는 현장 대응 참고서로 사용합니다.
...
> 운영 기준: `main` 브랜치는 FE 중심 실습 문맥이며, 백엔드 관련 트러블슈팅은 `feat/backend-2.0-working` 브랜치 기준으로 해석합니다.

```mermaid
flowchart TD
  A[문제 발생] --> B{로그인 상태?}
  B -- 아니오 --> C[로그인 후 재시도]
  B -- 예 --> D{Firestore 권한 설정?}
  D -- 아니오 --> E[Rules 수정]
  D -- 예 --> F{MQTT WSS 연결?}
  F -- 실패 --> G[브로커 URL/인증서 확인]
  F -- 성공 --> H[환경변수 점검]
```
