...
## 문서 목적
이 문서는 Phase 1 SDK 호출과 Phase 2 REST API 확장 지점을 함께 정의하는 인터페이스 명세 문서입니다.  
프런트엔드/백엔드 연동 포인트를 명확히 해 구현 경계를 합의하는 데 사용합니다.
...
Phase 1에서는 Firestore SDK를 직접 사용합니다. Phase 2에서는 REST API로 확장합니다.

## Phase 1 (SDK 기반)
- `onSnapshot(users/{uid}/devices)`
- `addDoc(users/{uid}/devices)`
- `updateDoc(users/{uid}/devices/{deviceId})`
