...
## 문서 목적
이 문서는 Phase 1 구조를 유지하면서 Phase 2 확장으로 연결하기 위한 리팩토링 절차를 안내하는 문서입니다.  
강의 실습에서 단계별 작업 순서와 체크포인트를 맞추는 운영 기준으로 사용합니다.
...
- [ ] 현재 디렉토리 구조 확인
  ...
  - 백엔드 확장 실습 시 `feat/backend-2.0-working` 브랜치 사용 여부 확인
...
### 📂 작업 디렉토리
```
(feat/backend-2.0-working 브랜치)
aiot-device-manager-be-working/
```
...
#### Step 1: 프로젝트 초기화 (1시간)
```bash
git checkout feat/backend-2.0-working
...
- `docs/강의 보조 자료/2.1_DEPLOYMENT_OPS_GUIDE.md` - 배포 가이드
- `docs/강의 보조 자료/1.3_SECURITY_AUTH.md` - 보안 검토
